  ...
  if (loopCounter == 0) {
    if (CURRENT_MODE == "USER_MODE") {
      ...
      if (debug == true) {
        ...
        Serial.println("Time is " + String(timeClient.getEpochTime()));
        ...
        if (coffeeBrewedTime != "") {
          Serial.println("Time is " + coffeeBrewedTime);
        }
        if (coffeeBrewedSeconds != NULL) {
          unsigned long passed = timeClient.getEpochTime() - coffeeBrewedSeconds;
          Serial.println("Time is " + String(passed));
        }
      }
      else
        Serial.println(":");
...
void StateChanged(State oldState, State newState) {
  ...
  if (oldState.getId() < newState.getId()) {
    ...
    if (coffeeRisesCounter >= COFFEERISES_INTERVAL) {
      coffeeBrewedTime = timeClient.getFormattedTime();
      coffeeBrewedSeconds = timeClient.getEpochTime();
      coffeeRises = true;
    }
  }
...
void SetConfigs() {
  ...
  if (debugLoad == 1)
    debug = true;
  byte timeZoneRawLoad = EEPROM.read(410);
  if (timeZoneRawLoad >= 0 && timeZoneRawLoad <= 14)
    timeZone = timeZoneRawLoad;
  if (timeZoneRawLoad >= 101 && timeZoneRawLoad <= 112)
    timeZone = 100 - timeZoneRawLoad;
  byte daylightSavingLoad = EEPROM.read(411);
  if (daylightSavingLoad == 0)
    daylightSaving = false;
  if (daylightSavingLoad == 1)
    daylightSaving = true;
}
...
void SetEndpoints() {
  ...
  server.on("/consts", HTTP_POST, []() {
    if (CURRENT_MODE == "CONFIG_MODE") {
      ...
      if (server.hasArg("dlsv")) {
        byte dlsv = (byte)server.arg("dlsv").toInt();
        if (dlsv >= 1 && dlsv <= 2)
          EEPROM.write(411, dlsv);
      }
      if (server.hasArg("tmzn")) {
        short tmznraw = (byte)server.arg("tmzn").toInt();;
        byte tmzn;
        if (tmznraw >= -12 && tmznraw <= 14) {
          if (tmznraw < 0)
            tmzn = fabs(tmznraw) + 100;
          else
            tmzn = tmznraw;
          EEPROM.write(410, tmzn);
        }
      }
      result = EEPROM.commit();
      if (result) {
...
String Layout(String MODE) {
  ...
  html += "</div>";
  html += "<footer>";
  html += "<p>Copyright © Vladislav Vishnevskii</p>";
  html += "</footer>";
  if (MODE == "USER_MODE") {
    if (coffeeBrewedSeconds != NULL) {
      unsigned long elapsedseconds = timeClient.getEpochTime() - coffeeBrewedSeconds;
      if (elapsedseconds < reloadTimer)
...
String Jug() {
  String html = "";
  if (coffeeBrewedTime != "")
    html += "<h2>Coffee brewed at " + coffeeBrewedTime + "</h2>";
  else
...
String Forms() {
  ...
  html += "<tr><td colspan='3'>Daylight saving: <input type='radio' name='dlsv' value='1'";
  if (dlsv == 2)
    html += " checked";
  html += ">enable<input type='radio' name='dlsv' value='0'";
  if (dlsv == 1)
    html += " checked";
  html += ">disable</td>";
  html += TimeZoneField();
  ...
  html += "</table>";
  html += "</form>";
  html += "<form action='/clear' method='POST' class='clearform'>";
...
String TimeZoneField() {
  byte tmznraw = EEPROM.read(410);
  String html = "<td colspan='2'>Time zone </td><td><select name='tmzn'>";
  for (int i = 0; i <= 112; i++) {
    String inx = "";
    if (i == 15)
      i = 101;
...
String Info() {
  ...
  html += "<p>Sensor settings allows to calibrate sensors to detect coffee better. You can ajust parameters, to suit light level of the room.<br> Theshold - amount of light passes to the sensor when the coffee on the sensor's level.<br> Range - the range of light levels which considered to mean that the coffee is on the sensor's level.<br> Max - the light level, above which the range of lightlevels only possible if jug is not present lies.</p>";
